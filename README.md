...
- **保险页面**
    ...
    - 第二部分为搜索栏和筛选功能
        - 搜索栏包含一个搜索框、一个搜索按钮、一个重置按钮以及一个筛选按钮
        - 搜索框对应搜索的product_name字段
        - 筛选按钮点击后弹出筛选面板，筛选面板动态显示当前保险产品类型的可筛选字段
            - 通过GET /insurance_products/product_fields获取保险产品类型对应字段列表
            - 筛选面板包含三类筛选：
                - **数值筛选**：支持数值类型字段，可选择比较运算符（>=、<=、>、<、=）并输入数值
                - **是/否筛选**：支持布尔类型字段，可选择是、否或不限
                - **文本搜索**：支持产品名称的模糊搜索
        - 搜索按钮和重置按钮分别对应搜索和重置，重置按钮清空搜索框和所有筛选条件
    - 第三部分为保险产品列表
        - 保险产品以卡片的形式展示，卡片包含产品名称、公司名称、产品类型等基本信息
        - 由于不同保险产品类型的字段不同，卡片会动态显示前3个可用字段的信息
        - 卡片右端有查看详情按钮，点击后转至具体保险产品信息页面
        - 卡片列表末尾有页码，可以通过点击页码或点击页码左右两端的切换页按钮来切换页，切换页会向后端请求对应页的保险产品列表
        - 卡片列表的上方有排序选择器，可以选择按照哪个数值类型字段进行排序（降序）
            - 排序字段列表通过GET /insurance_products/product_fields动态获取数值类型字段
            - 支持"默认排序"选项（不指定排序字段）
    - 保单相关部分
        - 在卡片列表下方，添加一个淡紫色的卡片为'我的保单'，点击后转至用户保单页面
    - **新添加的chat相关部分**：
        - 在'我的保单'上方，卡片列表页码下方，添加一个'产品对比'卡片，卡片包含两个中间带有'加号'的方框，方框下方有'产品对比'按钮。
        - 对保险卡片添加一个长按事件，长按后弹出底部菜单，菜单包含'进行对比'和'查看详情'按钮，点击'进行对比'后该产品加入到'产品对比'卡片中，在'产品对比'卡片中可以进行删除。
...
- **具体保险产品信息页面**
    ...
    - 页面显示产品基本信息卡片（产品名称、公司名称、产品类型）
    - 产品详情以动态字段的形式展示，支持不同保险产品类型的不同字段结构
        - 自动过滤掉空值和系统字段（如code、message、product_id等）
        - 字段内容根据数据类型自动格式化显示：
            - 数值类型：自动格式化（如大数字显示为"XX万"）
            - 布尔类型：显示为"是"或"否"
            - Map类型：以键值对形式展示
            - List类型：以列表形式展示
...
- 保险产品搜索API
    ...
    - 请求参数：
        ...
        - 其他可选筛选参数：根据不同产品类型有不同的字段可用于筛选
            - 数值类型字段：支持使用比较运算符筛选，格式为"运算符+数值"（如：>=18、<=65、>100等）
            - BOOLEAN类型字段：支持是/否筛选（传true或false）
            - 文本类型字段：支持模糊匹配（自动添加通配符）
    - 返回体：
        - code: 状态码
        - message: 搜索成功或失败信息
        - pages: 总页数
        - products: 保险产品列表（使用中文字段名返回，包含该产品类型的所有字段）
    - 注意：前端通过GET /insurance_products/product_fields获取字段信息，动态生成筛选和排序选项
